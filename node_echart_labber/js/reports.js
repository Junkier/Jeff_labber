...
function volumeTrends(data,mode){
    ...
    var legendType = mode === "all" ? ["總聲量"]: ["P/N比"] ;
    var allVolumeData = [],
        markPointData = [];

    days.map(function(d,index){
        if(mode==="all"){
            allVolumeData.push( data["by_days"][d]["post_vol_sum"].toFixed(0) );
            markPointData.push(  { coord : [ index,data["by_days"][d]["post_vol_sum"].toFixed(0) ]} );
        }else{
            allVolumeData.push(data["by_days"][d]["by_post_semantic"]["PN_ratio"].toFixed(1));
            markPointData.push(  { coord : [ index,data["by_days"][d]["by_post_semantic"]["PN_ratio"].toFixed(1) ] } );
        };
    });
...
function genTableHead(){
    var content = '<thead>'+
                      ...
                      '</thead>'+
                '<tbody></tbody>';
    $("#volSortTab1 table").append(content);
};

function createTable(data,rankParam){

    var place = "#keyword1 #volSortTab1 tbody";
