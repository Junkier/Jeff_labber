...
// 1. package.json - dependencies 裡，需加入 要 npm install 的 module
// 2. 上傳的 .zip 無需包含 node_modules
const PythonShell = require("python-shell");
const moment = require("moment");
const sizeof = require('object-sizeof');


exports.topicFptreeHandle = function topicFptreeHandle(req,res){
    let message = req.body.message;
    if(!message){
        res.status(400).send('No message defined!');
    } else if (message !== "trigger") {
        res.status(404).send(`Sorry ! ${message} not found.`);
    } else if (!req.body.keyword){
        res.status(400).send("Lack of keyword!");
    } else {
        fs.readFile('test_topic_fptree.json', 'utf8', function (err, data) {
            if (err) throw err;
            let fptree_raw_d = JSON.parse(data)["fptree"] ,
                keyword_list = typeof req.body.keyword === "string" ? [req.body.keyword] : req.body.keyword,
                t_start = new Date();
                mode = "test-gcf",
                chart_mode = "DashBoard";
            // 幹 keyword 要是 list 阿阿阿阿
            ...
            go_analyze({
                ...
                console.log(`topic fptree ---` +
                    ` ${req.body.keyword} 【mode : ${chart_mode} , time : ${(new Date() - t_start) / 1000} ` +
                ...
                console.log(fp_err);
                res.status(500).json({"Err":fp_err});
            });
        });
    }
}
